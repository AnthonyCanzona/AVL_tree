	...
	private:
		class Node {
			public:
				...
				int balanceFactor() { return (right_tree ? right_tree->height() + 1 : 0) - (left_tree ? left_tree->height() + 1 : 0);}
		};
        ...
        curr_node->update_height();
		if(curr_node->right_tree != nullptr){balance(curr_node->right_tree);}
		if(curr_node->left_tree != nullptr){balance(curr_node->left_tree);}
		}
    }
